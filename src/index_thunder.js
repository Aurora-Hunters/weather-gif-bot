...
const getFrame = async function (date, offset, place) {
    ...
    const imageUrl = `https://img4.meteologix.com/images/data/cache/model/model-en_modez_${LABEL}_${place}_155.png`
    const imagePath = path.join(get_directory(__dirname, '..', 'temp', 'thunder'), `${place}_${LABEL}.png`);
...
module.exports = async (place = PLACE.LEN) => {
     ...
     */
    try {
        const gifPath = path.join(get_directory(__dirname, '..', 'output'), `thunder_${place}_latest.mp4`);
